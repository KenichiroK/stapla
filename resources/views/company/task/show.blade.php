...
<div class="main__container">
    <div class="main__container__wrapper">
        ...
        <div class="actionButton">
            ...
            @elseif($task->status === 1 && $task->superior->id === $companyUser->id)
                <form action="{{ route('company.task.status.change') }}" method="POST">
                @csrf
                    <input type="hidden" name="task_id" value="{{ $task->id }}">
            ...
            @elseif($task->status === 2 && in_array($companyUser->id, $company_user_ids))
                <form action="{{ route('company.task.status.change') }}" method="POST">
                @csrf
                    <input type="hidden" name="task_id" value="{{ $task->id }}">
            ...
            @elseif($task->status === 10 && in_array($companyUser->id, $company_user_ids))
                <form action="{{ route('company.task.status.change') }}" method="POST">
                @csrf
                    <input type="hidden" name="task_id" value="{{ $task->id }}">
            ...
            @elseif($task->status === 13 && in_array($companyUser->id, $company_user_ids))
                <a href="{{ route('company.document.invoice.show', ['id' => $invoice->id]) }}" class="done">請求書を確認する</a>
            @elseif($task->status === 15 && $task->accounting->id === $companyUser->id)
                <a href="{{ route('company.document.invoice.show', ['id' => $invoice->id]) }}" class="done">請求書を確認する</a>
            @elseif($task->status === 16 && in_array($companyUser->id, $company_user_ids))
                <form action="{{ route('company.task.status.change') }}" method="POST">
                @csrf
                    <input type="hidden" name="task_id" value="{{ $task->id }}">
