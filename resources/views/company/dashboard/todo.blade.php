<div class="todo-container">
    <div class="title-container">
        ...
        <p class="title-container__alert">
            ...
            <button id="toggle_todo_btn" class="title-container__alert--button" type="button">
                {{ count($after_three_days_todos) }}<span class="title-container__alert--unit">件</span>
            </button>
        </p>
    </div>
    ...
    <div class="content-container">
        ...
        <div id="all_todos">
            ...
            <div class="content-container__body todo_item">
                <a class="content-container__body--link" href="{{ route('company.task.show', ['id' => $todo->id]) }}">
                    ...
                    <p class="content-container__body--long">
                        {{ date('n月j日', strtotime($todo->status_udpated_at)) }}
                        @if (\Carbon\Carbon::now()->diffInDays(new \Carbon\Carbon($todo->status_udpated_at)) > 3)
                            <br>
                            <span class="date-alert">依頼から{{ \Carbon\Carbon::now()->diffInDays(new \Carbon\Carbon($todo->status_udpated_at)) }}日経っています</span>
                        @endif
                    </p>
        ...
        <div id="after_three_days_todos">
            @foreach($after_three_days_todos as $todo)
            <div class="content-container__body todo_item">
                <a class="content-container__body--link" href="{{ route('company.task.show', ['id' => $todo->id]) }}">
                    <p class="content-container__body--long">{{ $todo->project->name }}</p>
                    <p class="content-container__body--long">{{ $todo->name }}</p>
    ...
    <div id="todo_more_btn_area" class="morebtn-container is-active">
        <p id="todo_more_btn" class="morebtn-container__text">もっと見る</p>
    </div>
</div>
